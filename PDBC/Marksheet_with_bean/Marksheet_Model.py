...
class Marksheet_Model():
    ...
    def insert(self,bean : Marksheet_Bean):
        id = Marksheet_Model.next_Primary_Key(self)
        roll_no = bean.get_roll_no()
        name = bean.get_name()
        phy = bean.get_phy()
        che = bean.get_che()
    ...
    def get(self,id):
        ...
        cursor.execute(sql,id)
        column = ['id','roll_no','name','phy','che','maths']
        list = []
        data = cursor.fetchall()
        for row in data:
            list.append({column[i]:row[i] for i, _ in enumerate(row)})
        connection.commit()
        connection.close()
        return list
